...
def create_dataframe_from_tweet_list(tweet_list: list) -> pd.DataFrame:
    tweets_df = pd.DataFrame(tweet_list, columns=['id', 'date', 'user_name', 'user_id', 'user_displayname', 'user_location',
                                                  'user_followers', 'user_friends', 'user_statuses', 'retweeted', 'content', 'lang', 'is_missinginfo'])
...
for i, tweet in enumerate(sntwitter.TwitterSearchScraper(f"{key_words} since:2021-01-01 until:2021-05-31").get_items()):
    ...
    temp_tweets_list.append([tweet.id, tweet.date, tweet.user.username, tweet.user.id, tweet.user.displayname, tweet.user.location,
                             tweet.user.followersCount, tweet.user.friendsCount, tweet.user.statusesCount,
                             tweet.retweetedTweet, tweet.rawContent, tweet.lang, None])
